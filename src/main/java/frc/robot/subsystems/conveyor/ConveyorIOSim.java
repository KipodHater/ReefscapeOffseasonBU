...
import frc.robot.subsystems.conveyor.ConveyorIO.ConveyorIOInputs;

public class ConveyorIOSim implements ConveyorIO {
  private SparkMax baseMotor;
  private SparkMaxSim motor;
  private SparkMaxConfig config;

  //   private RelativeEncoder motorEncoder;
  private SparkRelativeEncoderSim motorEncoder;
  ...
  public ConveyorIOSim() {
    ...
    baseMotor = new SparkMax(MOTOR_ID, MotorType.kBrushless);
    ...
    motor = new SparkMaxSim(baseMotor, DCMotor.getFalcon500(1));
    motorEncoder = motor.getRelativeEncoderSim();
  }
  ...
  public void updateInputs(ConveyorIOInputs inputs) {
    ...
    inputs.velocityDegPerSec = motorEncoder.getVelocity();
    inputs.motorVoltage = motor.getAppliedOutput() * motor.getBusVoltage();
    inputs.motorTemp = 0;
    inputs.motorCurrent = motor.getMotorCurrent();

    inputs.motorConnected = connectedDebouncer.calculate(true);
  }

  @Override
  public void runVoltage(double voltage) {
    motor.setBusVoltage(MathUtil.clamp(voltage, -12, 12));
  }
  ...
  public void checkCurrent() {
    ...
    overCurrent = motor.getMotorCurrent() > currentThreshold;
