...
public class ClimbIOSpark implements ClimbIO {
  ...
  public ClimbIOSpark() {
    ...
    climbPIDController =
        new ProfiledPIDController(GAINS.KP(), GAINS.KI(), GAINS.KD(), CLIMB_CONSTRAINTS);
  }
  ...
  public void updateInputs(ClimbIOInputs inputs) {
    ...
    ifOk(
        ...
        (position) ->
            inputs.positionDeg =
                (position - CLIMB_ENCODER_OFFSET) > 180
                    ? (position - CLIMB_ENCODER_OFFSET) % 360 - 360
                    : (position - CLIMB_ENCODER_OFFSET) % 360);
  ...
  public void runPosition(double position, double feedforward) {
    ...
    motor.setVoltage(
        climbPIDController.calculate(climbEncoder.getPosition(), position) + feedforward);
  }
